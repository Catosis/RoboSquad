	...
	if selectedGroup == -1:
		for index, robot in enumerate(robots):
			if robot.agent != -1:
				...
				print "All Robot " + str(index) + " velocity set to " + str(sim.getAgentPrefVelocity(robot.agent))
	else:
		for index, robot in enumerate(groups[selectedGroup]):
			if robot.agent != -1:
				coords = canvas.coords(robot.robotShapeID) #SIMULATED perfect position
				x_vec = goalx - (coords[0]) #simulated vector from center
				y_vec = goaly - (coords[1])
				...
				sim.setAgentPrefVelocity(robot.agent, (x_vec,y_vec))
				#sim.setAgentPrefVelocity(robot.agent, (1, 1))
				print "Grouped Robot " + str(index) + " velocity set to " + str(sim.getAgentPrefVelocity(robot.agent))
...
def selectRobot(num):
	#toggle the button on that robot
	currentRobot = robots[num]
	if currentRobot.toggled:
		currentRobot.button.config(bg="light grey")
	else:
		currentRobot.button.config(bg="indian red")
	currentRobot.toggled = not currentRobot.toggled
	print robots[num].toggled
	return

def groupRobots():
	#iterate through buttons and see which one are toggled
	#change the color of the buttons
	thisGroup = []
	groupNum = len(groups)
	for index, robot in enumerate(robots):
		if robot.toggled:
			robot.toggled = not robot.toggled
			robot.button.config(state="disabled", bg="grey")
			thisGroup.append(robot)
			robot.group = groupNum
			print str(robot.group)
	string =  ', '.join([str(robot.name) for robot in thisGroup])
	Tkinter.Label(groupListWindow, text=string, bg="white", padx=25).grid(row=7,column=0, sticky="nwes")
	goalButton = Tkinter.Button(groupListWindow, text="Define Goal", command = defineGroupGoal(groupNum))
	goalButton.grid(row=7, column=2, sticky="n")
	groups.append(thisGroup)

def ungroupRobots(groupNum):
	return

def defineGroupGoal(groupNum):
...
groups = [""] # array of arrays containing the robots #just keep tracks of how many groups there are
global robot_pos
afterID = -1
selectedGroup = -1
...
o1 = sim.addObstacle([(0, 0), (800, 0), (800, 600), (0, 600)])
...
start_button = Tkinter.Button(frame, text="Start", command=lambda: initializeRobot(0))
start_button.grid(row=0, column=0, sticky='we', columnspan = 2)
quit_button = Tkinter.Button(frame, text="Quit", command=cleanCloseProgram)
quit_button.grid(row=0, column=2, sticky='we', columnspan = 2)
Tkinter.Label(frame, text="SWARM CONTROL PANEL").grid(row=1, sticky="W", padx=20, pady=20, columnspan=4)
robot0.button = Tkinter.Button(frame, text=" R1 ", command = lambda:selectRobot(0), bg ="light grey", highlightcolor = "orange")
robot1.button = Tkinter.Button(frame, text=" R2 ", command = lambda:selectRobot(1), bg ="light grey", highlightcolor = "orange")
robot2.button = Tkinter.Button(frame, text=" R3 ", command = lambda:selectRobot(2), bg ="light grey", highlightcolor = "orange")
robot3.button = Tkinter.Button(frame, text=" R4 ", command = lambda:selectRobot(3), bg ="light grey", highlightcolor = "orange")
robot0.button.grid(row=3, column=0, sticky = "wens")
robot1.button.grid(row=3, column=1, sticky = "wens")
robot2.button.grid(row=3, column=2, sticky = "wens")
robot3.button.grid(row=3, column=3, sticky = "wens")
Tkinter.Button(frame, text="Group", command = lambda:groupRobots()).grid(row=4, column=1, columnspan=2, stick="we", pady=2)
Tkinter.Label(frame, text="--Groups--", bd=3, relief="ridge").grid(row=5,column=0, columnspan = 4, sticky="nwes", pady=5)
groupListWindow=Tkinter.Frame(frame, bg="white", bd=2, relief="sunken", height=100, padx = 50)
groupListWindow.grid(row=6,column=0, columnspan = 4, sticky="nwes")
Tkinter.Label(groupListWindow, text="no groups yet", bg="white", padx=25).grid(row=7,column=0, sticky="nwes")
var = Tkinter.StringVar(root)
var.set("none") # initial value
optionInput=Tkinter.OptionMenu(groupListWindow, var, "none", "ungroup")
#label3=Label(frame, text="Y").grid(row=2,column=0, sticky="w")
optionInput.grid(row=7,column=5,sticky="e")
...
#####END OF GUI SETUP#####
#####
#note0: initialization problem because currently patched with if statement (doesnt init all
# the robots if there's less than four, leaving the default of -1)
...
#add numbers to the images: http://effbot.org/tkinterbook/canvas.htm#item-specifiers
#######
...
def step(goalx, goaly):
	...
	for index, robot in enumerate(robots):
		if robot.agent != -1:
			...
			dutyCycles = convertToDutyCycles(agentVelocity[0], agentVelocity[1], robot.irl_angle)
...
def convertToDutyCycles(vx, vy, angle):
	...
	#w1_max = -x_max;
	#w2_max = (0.5 * x_max - math.sqrt(1.5) * y_max);
	#w3_max = (0.5 * x_max + math.sqrt(1.5) * y_max);
